...
loopenddate=$(/bin/date --date "2022-01-31 1 day" +%Y-%m-%d)
...
do
	...
	hh="$(printf "%02d" $h)"
 	...
 	do
	    ...
	    echo " "python $dir_/demo_satpy_fog.py $year $month $d $h $m 
 	    ${python} $dir_/demo_satpy_fog.py $year $month $d $h $m 
	    #echo " "
