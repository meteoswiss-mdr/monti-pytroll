	...
	do 
		...
		hh="$(printf "%02d" $h)"
 		...
 		do
		    ...
		    #echo " "
		    #$python ${dir1}/produce_forecasts_nrt.py $year $month $d $h $m
		    #dir1=$dir0/${year}-${MM}-${dd}

		    echo ${python} $dir_/plot_msg.py input_ot_anvil_detection $year $month $d $h $m 
 		    ${python} $dir_/plot_msg.py input_ot_anvil_detection $year $month $d $h $m 
