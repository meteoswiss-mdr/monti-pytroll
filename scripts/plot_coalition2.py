...
if __name__ == '__main__':
    ...
    # always switch on cleaning for NRT processing and local area
    if scale != "europe" and len(sys.argv) < 2:
        print "... switch on cleaning with skimage"
        clean_mask = 'skimage'

    # forth mask (small ice crystal test)
    forth_mask  = 'IR_039_minus_IR_108'      # 'CloudType'           # 'no_mask'

    # mask for forcing additional pixels (not active at the moment)
    in_msg.forced_mask = 'no_mask'                  # 'IR_039_minus_IR_108' # 'CloudType' #
    mask_cirrus = True

    if in_msg.rapid_scan_mode:
    ...
    # chose time interval for estimating the updraft 
    # default
    dt_forecast1 = 15  # min
    dt_forecast2 = 30  # min

    if in_msg.rapid_scan_mode == True:
    ...
    while time_slot <= time_slotSTOP:
          ...
          # fore the brightness temperatures to the current (observed) cloud mask 
          ...
          if 'mature_mask' in in_msg.aux_results:
              outputFile = in_msg.outputDir +"/cosmo/Channels/indicators_in_time/masks/"+yearS+monthS+dayS+"_"+hourS+minS+"_"+"4th"+name_4Mask+"_Mature_mask.png"
          ...
          if developing_mask in in_msg.aux_results:
              outputFile = in_msg.outputDir +"/cosmo/Channels/indicators_in_time/masks/"+yearS+monthS+dayS+"_"+hourS+minS+"_4th"+name_4Mask+"_Developing_mask.png"
          ...
          if in_msg.forced_mask == 'CloudType':
              for ct in range(0,len(cloud_type_forced)):
                  cl_typ = cloud_type_forced[ct]
                  force_mask = np.where(data['CT'].data == cl_typ,1,force_mask)
          elif in_msg.forced_mask == 'IR_039_minus_IR_108':  
                  force_mask = np.where(IR_039_minus_IR_108 >= force_th_chDiff,1,0)
          elif in_msg.forced_mask !='no_mask': 
              print "*** Error in main (Mecikalski_test.py)"
              print "    unknown forcing mask -> applying no forcing mask", in_msg.forced_mask     
      
          if 'forced_mask' in in_msg.aux_results:
              outputFile = in_msg.outputDir +"/cosmo/Channels/indicators_in_time/masks/"+yearS+monthS+dayS+"_"+hourS+minS+"_4th"+name_4Mask+"_Forced_mask.png"
          ...
          if final_mask in in_msg.aux_results:
              ...
              outputFile = in_msg.outputDir +"/cosmo/Channels/indicators_in_time/masks/"+yearS+monthS+dayS+"_"+hourS+minS+"_4th"+name_4Mask+"_"+name_ForcedMask+"AdditionalMask_Final_mask.png"
          ...
          metadata['rapid_scan_mode'] = in_msg.rapid_scan_mode
          ...
          if 'labelled_objects' in in_msg.aux_results:
                outputFile = in_msg.outputDir +"/cosmo/Channels/indicators_in_time/labelled/"+yearS+monthS+dayS+"_"+hourS+minS+"_4th"+name_4Mask+"_"+name_ForcedMask+"AdditionalMask.png"
          ...
          #c2File = (in_msg.outputDir+"/cosmo/Channels/indicators_in_time/RGB"+maskS+"/%s_%s_C2rgb"+maskS+"4th"+name_4Mask+"_"+name_ForcedMask+"AdditionalMask.png") % (yearS+monthS+dayS,hourS+minS)
