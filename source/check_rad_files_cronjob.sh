...
if [ 1 -eq 1 ]; then
    ...
    date_start="2023-04-01 00:00"
    date_start_s=$(/bin/date --date "$date_start" +"%s")
    date_end="2023-04-01 00:05"
...
do
	...
	cd /opt/users/hau/monti-pytroll/source/  # necessary to specify input file without path
	if [ $# -eq 0 ]; then
	    echo "not yet implemented!!!"
	elif [[ "$1" = "PLAX" ]]; then
	    echo "    "python /opt/users/hau/monti-pytroll/source/demo_parallax.py input_parallax.py --date $year $month $day $hour $minute
	    python /opt/users/hau/monti-pytroll/source/demo_parallax.py input_parallax.py --date $year $month $day $hour $minute
	else
	    echo "unknown command line arguemnt" $1
	fi		    
    fi
