...
def input(in_msg, timeslot=None):
    ...
    if in_msg.nrt:
        ...
        # for FDS, we use EUMETSAT products
        if in_msg.sat_nr==10:
            in_msg.base_dir_ctth = "/data/cinesat/in/eumetcast1/"     # /data/cinesat/in/eumetcast1/S_NWC_CTTH_MSG4_MSG-N-VISIR_20230224T160000Z.nc
            in_msg.base_dir_ct   = "/data/cinesat/in/eumetcast1/"
        # for RSS, we use NWCSAF product processed by MCH
        elif in_msg.sat_nr==11:
            in_msg.base_dir_ctth = "/data/cinesat/in/safnwc/"         # /data/cinesat/in/safnwc/    S_NWC_CTTH_MSG3_alps-VISIR_20230224T160000Z.nc
    ...
    ### in_msg.parallax_gapfilling = 'bilinear' # 'False' (default), 'nearest'   # only mpop implementation 
    #in_msg.save_reprojected_data=['ccs4']
    in_msg.msg_pc_filename='%(msg)s_%(area)s_%Y%m%d%H%M_rad_PLAX.nc'
    in_msg.msg_pc_data_dir='/data/COALITION2/database/meteosat/ccs4_PLAX/%Y/%m/%d/'
    #in_msg.msg_pc_data_dir='/data/cinesat/out/'
    #in_msg.nwc_pc_filename='S_NWC_radiance_%(msg)s_%(area)s_%Y%m%d%H%M_PLAX.nc'
    #in_msg.nwc_pc_data_dir='/data/COALITION2/database/meteosat/SAFNWC_v2016/%Y/%m/%d/our_PLAX/'
    
    
    # specify SEVIRI L1.5 channels, which you like to save as parallax corrected
    # --------------------------------------------------------------------------
    in_msg.msg_pc = []
    in_msg.msg_pc.append('VIS006')
    in_msg.msg_pc.append('VIS008')
    in_msg.msg_pc.append('IR_016')
    in_msg.msg_pc.append('IR_039')
    in_msg.msg_pc.append('WV_062')
    in_msg.msg_pc.append('WV_073')
    in_msg.msg_pc.append('IR_087')
    in_msg.msg_pc.append('IR_097')
    in_msg.msg_pc.append('IR_108')
    in_msg.msg_pc.append('IR_120')
    in_msg.msg_pc.append('IR_134')
    in_msg.msg_pc.append('HRV')

    # specify NWCSAF products, which you like to save as parallax corrected
    # --------------------------------------------------------------------------
        ## NWC SAF
    ##-------------------
    in_msg.nwcsaf_pc = []
    ## NWC SAF PEG 1
    #in_msg.nwcsaf_pc.append('CMa')
    #in_msg.nwcsaf_pc.append('CMa_DUST')
    #in_msg.nwcsaf_pc.append('CMa_VOLCANIC')
    #in_msg.nwcsaf_pc.append('CMa_QUALITY')
    ## NWC SAF PEG 2
    #in_msg.nwcsaf_pc.append('CT')
    #in_msg.nwcsaf_pc.append('CT_PHASE')
    #in_msg.nwcsaf_pc.append('CT_QUALITY')
    ## NWC SAF PEG 3 
    in_msg.nwcsaf_calibrate=True
    #in_msg.nwcsaf_pc.append('CTT')
    #in_msg.nwcsaf_pc.append('CTH')
    #in_msg.nwcsaf_pc.append('CTP')
    ## NWC SAF PEG 13 
    #in_msg.nwcsaf_calibrate=False
    #in_msg.nwcsaf_pc.append('sphr_bl')
    #in_msg.nwcsaf_pc.append('sphr_cape')
    #in_msg.nwcsaf_pc.append('sphr_diffbl')
    #in_msg.nwcsaf_pc.append('sphr_diffhl')
    #in_msg.nwcsaf_pc.append('sphr_diffki')
    #in_msg.nwcsaf_pc.append('sphr_diffli')
    #in_msg.nwcsaf_pc.append('sphr_diffml')
    #in_msg.nwcsaf_pc.append('sphr_diffshw')
    #in_msg.nwcsaf_pc.append('sphr_difftpw')
    #in_msg.nwcsaf_pc.append('sphr_hl')
    #in_msg.nwcsaf_pc.append('sphr_ki')
    #in_msg.nwcsaf_pc.append('sphr_li')
    #in_msg.nwcsaf_pc.append('sphr_ml')
    #in_msg.nwcsaf_pc.append('sphr_quality')
    #in_msg.nwcsaf_pc.append('sphr_sflag')
    #in_msg.nwcsaf_pc.append('sphr_shw')
    #in_msg.nwcsaf_pc.append('sphr_tpw')
    ...
    #----------------------
    # choose RGB images that the program should produce 
    #----------------------
    ## satellite channels
