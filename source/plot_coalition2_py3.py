...
#from plot_msg_py3 import create_PIL_image
#from plot_msg_py3 import add_borders_and_rivers
from plot_msg_py3 import add_title
...
#from properties_cells_py3 import properties_cells
...
from produce_forecasts_nrt_py3 import downscale
...
def make_figure(values, obj_area, outputFile, colorbar=True, text_to_write=None,
                vmin=False, vmax=False, contour_value=None, linewidth=1):
...
def plot_coalition2(in_msg, time_slot, time_slotSTOP):
    ...
    while time_slot <= time_slotSTOP:
          ...
          for i_try in range(30):
              ...
              RGBs = check_input(in_msg, in_msg.sat_str()+in_msg.sat_nr_str(), in_msg.base_dir_sat, in_msg.datetime, RGBs=in_msg.RGBs)
              if len(RGBs) > 0:
          ...
          #global_data_CTP = create_seviri_scene(in_msg.datetime, in_msg.base_dir_ctth, sat=MSG_str, reader='nwcsaf-geo')
          ...
          for area in in_msg.areas:
                ...
                if chosen_settings['scale'] == 'local' and in_msg.no_NWCSAF == False:
                    ...
                    for i_try in range(30):
                        # check if "ctth_alti" file is present
                        RGBs = check_input(in_msg, in_msg.sat_str()+in_msg.sat_nr_str(), in_msg.base_dir_ctth, in_msg.datetime, RGBs="CTH")
                        if len(RGBs) > 0:
                            # exit loop, if input is found
                            break
                        else:
                    ...
                    global_data_CTH = create_seviri_scene(in_msg.datetime, in_msg.base_dir_ctth, sat=MSG_str, reader='nwcsaf-geo')
                    with warnings.catch_warnings(): 
                        warnings.simplefilter("ignore")
                        global_data_CTH.load(['ctth_alti'])  # "Cloud Top Height"
                    data_CTH = global_data_CTH.resample(area, precompute=True)
                
                print ('... project data to desired area ', area)
                data = global_data.resample(area, precompute=True)

                sat_name = data['IR_108'].attrs['platform_name']
                sat_nr   = int(re.findall(r'\d+', data['IR_108'].attrs['platform_name'])[0])
                
                # fill placeholders in directory names with content
                outputDir = format_name(in_msg.outputDir, time_slot, area=area, rgb=rgb_name, sat=sat_name, sat_nr=sat_nr) # !!! needs change
                labelsDir = format_name(in_msg.labelsDir, time_slot, area=area, rgb='label',  sat=sat_name, sat_nr=sat_nr) # !!! needs change
                ...
                if chosen_settings['forth_mask'] == 'IR_039_minus_IR_108':
                    ...
                    if (scale == "local"):
                        global_CT_data = create_seviri_scene(time_slot, in_msg.base_dir_ct, sat=MSG_str, reader='nwcsaf-geo')
                        
                    elif (scale == "broad"):    
                        from my_msg_module import get_last_SEVIRI_date
                        time_slot_FDS = get_last_SEVIRI_date(False, time_slot=time_slot) # False -> No RSS (i.e. FDS == Full Disk Service)
                        global_CT_data = create_seviri_scene(time_slot_FDS, in_msg.base_dir_ct, sat=MSG_str, reader='nwcsaf-geo')
                    ...
                    with warnings.catch_warnings():
                        warnings.simplefilter("ignore")
                        global_CT_data.load(['ct'])  # "CT" for loading the cloud type RGB image

                    CT_data = global_CT_data.resample(area, precompute=True)
                ...
                if chosen_settings['mode_downscaling']:                    
                    if "CTH" in loaded_channels:
                        print ("    take CTH mask for downscaling")
                        mask_downscale = data_CTH["ctth_alti"].data.mask 
                        data = downscale(data, mode=chosen_settings['mode_downscaling'], mask=mask_downscale)
                    else:
                        ...
                        data = downscale(data, mode=chosen_settings['mode_downscaling'])
                ...
                print("create a cloud mask") # if scale local based on CTH, else based on where CTP can be derived
                # -------------------
                ...
                if chosen_settings['scale'] != 'local' or in_msg.no_NWCSAF == True:
                    ...
                    # every value less than 1 is masked (to make sure 0 is also masked)
                    ## data_CTH["ctth_alti"].data = ma.masked_less(data_CTH["ctth_alti"].data.compute(), 1) # !!! does not work any more: copying masked array to numpy array, the result is numpy array !!! 
                    ## mask_NoClouds = data_CTH["ctth_alti"].data.mask                                      # !!! does not work any more: copying masked array to numpy array, the result is numpy array !!! 
                    mask_NoClouds = ma.masked_less(data_CTH["ctth_alti"].data, 1).mask

                nowcastDir = format_name(in_msg.nowcastDir, time_slot, area=area, rgb='channels', sat=sat_name, sat_nr=sat_nr) # !!! needs change
                print(in_msg.nowcastDir, in_msg.nrt)
                ...
                if chosen_settings['use_TB_forecast'] == True:
                    ...
                    global_data30 = create_seviri_scene(time_slot30, in_msg.base_dir_sat, sat=MSG_str)
                    with warnings.catch_warnings():
                        warnings.simplefilter("ignore")
                        global_data30.load(in_msg.channels30)
                    ...
                    data30 = global_data30.resample(area, precompute=True)
                    if chosen_settings['mode_downscaling']: 
                    ...
                    global_data15 = create_seviri_scene(time_slot15, in_msg.base_dir_sat, sat=MSG_str)
                    with warnings.catch_warnings():
                        warnings.simplefilter("ignore")
                        global_data15.load(in_msg.channels15)
                    ...
                    print("    resample to ", area)
                    data15 = global_data15.resample(area, precompute=True)
                    if chosen_settings['mode_downscaling']: 
                ...
                print("*** force the brightness temperatures to the current (observed) cloud mask") 
                ...
                print("*** calculate threshold tests for CLOUD OPTICAL DEPTH")
                # -------------------
                ...
                print("*** calculate threshold test for GLACIATION")
                # -------------------
                ...
                print("*** calculate threshold test for UPDRAFT STRENGTH ")
                # -------------------
                ...
                print("*** calculate threshold test for SMALL ICE CRYSTALS (this test works only at daytime)")
                # -------------------------------------------------------------
                ...
                if chosen_settings['forth_mask'] != 'no_mask': 
                    ...
                    si = np.zeros( (nx,ny) )
                    si = np.where( data['IR_108'].data.compute() > 0, si+1, si)  # just a mask to get rid of the space outside the MSG disk
                    mature_mask += si
                    mature_mask = np.where( mature_mask==1, 1, 0) # start counting at -2  (so 1 is mature thunderstorms = -2+1+1+1)
                ...
                if chosen_settings['forced_mask'] == 'CloudType':
                    for ct in range(0,len(cloud_type_forced)):
                        ...
                        force_mask = np.where(data['CT'].data == cl_typ,1,force_mask)          ### ??? origninally data['CT'] could data be replaced by CT_data ???  
                ...
                if in_msg.parallax_correction:
                    ...
                    c2_data = create_seviri_scene(time_slot, in_msg.base_dir_sat, sat=MSG_str)
                    ...
                    #print ("type(data_CTH["ctth_alti"].data)",type(data_CTH["ctth_alti"].data.data), data_CTH["ctth_alti"].data.dtype, data_CTH["ctth_alti"].data.shape )
                    #c2_data.channels.append(Channel(name='CTH', wavelength_range=[0.,0.,0.], resolution=1000., data=data_CTH["ctth_alti"].data.data, area=area))
                    ...
                    c2_data = c2_data.parallax_corr(cth=data_CTH["ctth_alti"].data, fill=in_msg.parallax_gapfilling, estimate_cth=in_msg.estimate_cth, replace=True)
                    r    = c2_data['r'].data
                    g    = c2_data['g'].data
                    b    = c2_data['b'].data
                ...
                c2File = format_name(outputDir+'/'+in_msg.outputFile, time_slot, area=area, rgb=rgb_name, sat=sat_name, sat_nr=in_msg.sat_nr)
                ...
                if 'C2rgb' in in_msg.results:
                    ...
                    if in_msg.HRV_enhancement:
                        ...
                        #from mpop.imageo.geo_image import GeoImage
                        luminance = GeoImage((data["HRV"].data), data.area, time_slot, crange=(0, 100), mode="L")
                        luminance.enhance(gamma=2.0)
                        # add luminance to image
                        img = pilimage2geoimage(PIL_image, data.area_def, time_slot)
                    ...
                    if 'ninjotif' in in_msg.outputFormats:
                        ...
                        print ("... resample COALITION2 result ("+area+") to "+area_ninjotif)
                        ...
                        c2_data = c2_data.resample(area_ninjotif, precompute=True) # (default) mode='quick' || mode="bilinear", radius=50e3 || mode="ewa" || mode="nearest"

                        """
                        # !!! FIX ME !!!
                        # resample all channels to the area needed by NINJO
                        if area_ninjotif == 'nrEURO3km':
                            # normal resampleion
                            c2_data = c2_data.resample(area_ninjotif, precompute=True) 
                        elif area_ninjotif == 'nrEURO1km':
                            # take care of area outside of ccs4, which is filled with nearest neighbour...
                            c2_data = c2_data.resample(area_ninjotif, mode='quick', precompute=True) # , mode='quick' || mode="bilinear", radius=50e3 || mode="ewa" || mode="nearest"
                        ...
                        c2ninjotif_file = format_name (outputDir+'/ninjo/'+in_msg.ninjotifFilename, time_slot, sat_nr=in_msg.sat_nr_str(), RSS=True, area=area_ninjotif )
                        print ("... save ninjotif image: display ", c2ninjotif_file, " &")
                        PIL_image = Image.fromarray(rgbArray2, 'RGBA')
                        GEO_image = pilimage2geoimage(PIL_image, c2_data['r'].area_def, time_slot)
                        if area_ninjotif == 'nrEURO1km':
                            ...
                            GEO_image.save(c2ninjotif_file,
                                           ...
                                           ninjo_product_name='COALITION2', chan_id=chan_id,
                                           nbits=8)   
                        os.chmod(c2ninjotif_file, 0o777)
                ...
                if area in in_msg.postprocessing_areas:                        
                    ...
                    postprocessing(in_msg, time_slot, int(in_msg.sat_nr_str()), area)
          ...
          for file_per in filenames_for_permission:
              try:
                  ...
                  print("*** modified permission for files: ", file_per)
              except PermissionError:
                  continue

          # increase time by 5min and run again 
